...
/// Helper: extract the first function from a Go source and return its events.
#[cfg(test)]
pub(crate) fn go_first_fn_events(source: &str) -> Vec<crate::ir::events::QualitasEvent> {
    ...
    assert!(
        ...
        "Expected at least one function or method in Go source",
    );
    extraction
        .classes
        .into_iter()
