...
import { analyzeSource, quickScore } from '../../js/index.js';
...
function f(a: any, b: any, c: any, d: any, e: any) {
  ...
  it('ScoreBreakdown penalties sum to total penalty', () => {
    const src = `
...
describe('quickScore', () => {
  it('returns compact shape with correct fields', () => {
  ...
  it('score matches analyzeSource for same input', () => {
    const src = `
...
`;
  ...
  it('returns topFlags for code with high complexity indicators', () => {
    const src = `
...
`;
  ...
  it('clean code returns score >= 80 and empty topFlags', () => {
...
describe('renderFileReport — scope filtering', () => {
  ...
  it('scope=function (default) includes function names', () => {
    const src = `
...
`;
  ...
  it('scope=file omits function names but shows file score', () => {
    const src = `
export function alpha(x: number) { return x; }
export function beta(x: number) { return x * 2; }
`;
  ...
  it('scope=class shows class summary, skips standalone functions', () => {
    const src = `
...
`;
  ...
  it('default scope (no option) behaves like scope=function', () => {
